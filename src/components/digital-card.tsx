...
type IconName = "phone" | "whatsapp" | "mail" | "location" | "linkedin" | "download" | "share" | "qr";
...
export function DigitalCard({ card }: { card: CardData }) {
  ...
  useEffect(() => {
    ...
    const close = (e: KeyboardEvent) => { if (e.key === "Escape") { setQrOpen(false); setContactHelpOpen(false); } };
  ...
  async function saveContact() {
    ...
    try {
      ...
      const file = new File([await response.blob()], `${card.slug}.vcf`, { type: "text/vcard" });
    ...
    <main className="page-shell">
      <article className="card-shell">
        <section className="hero">
          <img src="/images/infinitum-network.svg" alt="" className="network-bg" aria-hidden="true" />
        ...
        <section className="section">
          <p className="eyebrow">CONTACT DETAILS</p><h3>Connect directly</h3>
        ...
        <section className="panel social-panel"><h3>Connect Directly</h3><div className="social-list">{card.socialLinks.map((link) => <a key={link.platform} href={link.url} target="_blank" rel="noreferrer"><IconBox name="linkedin" /><span><b>{link.label}</b><small>{link.handle}</small></span><em>›</em></a>)}</div></section>

        <section className="panel qr-panel"><p className="eyebrow">ONE SCAN. INSTANT ACCESS.</p><h3>Share this digital card</h3><p>Let anyone save Satyadev&apos;s details in seconds.</p><button onClick={() => setQrOpen(true)}><AppIcon name="qr" /> Show QR Code</button></section>
